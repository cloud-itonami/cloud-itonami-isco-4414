(ns scribe.advisor
  "ScribingAdvisor — the advisor named in this repository's README,
  proposing a document-preparation operation (draft a document,
  transcribe dictation, certify a copy as a true copy, file a
  document on a client's behalf, sign on a client's behalf) for a
  registered scribing client. The advisor is swappable: `mock-advisor`
  ...
  of client provenance, delegated-authority scope or attestation/
  legal-filing risk; `scribe.governor` is the independent system that
  decides whether the proposal may proceed, per the itonami actor
  pattern. Modeled on cloud-itonami-isco-4214's debtcollection.advisor.

  A proposal is a map:
    {:op :draft-document|:transcribe-dictation|:certify-copy|
...
(def ^:private system-prompt
  "You are a scribing advisor. Given a document-preparation operation
   request, propose an :op, an honest :confidence (0.0-1.0), and a
   :stake (:low/:medium/:high). Never fabricate confidence you don't
   have. Certifying a copy, filing on a client's behalf and signing on
   a client's behalf always require human sign-off regardless of
   confidence — the governor checks delegated authority and
   attestation risk independently.")
