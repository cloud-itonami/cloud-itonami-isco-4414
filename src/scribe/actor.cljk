(ns scribe.actor
  "ScribingActor — the ISCO-08 4414 independent scribing & document-
  preparation practice actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  document-preparation operation request (intake → advise → govern →
  ...
  The unconditional invariant: the ScribingAdvisor can never directly
  file, sign, certify or commit a record the ScribingGovernor refuses
  — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [scribe.advisor :as advisor]
            [scribe.governor :as governor]
            [scribe.store :as store]))

(defn build-graph
  "Build a compiled ScribingActor graph. `store` implements
  `scribe.store/Store`. `advisor` implements `scribe.advisor/Advisor`
